% Dhruv Tyagi (2K19/EP/032) & Ayush Kumar (2K19/EP/030)
%
% Simulation of a particle moving in a Gaussian beam trap
%
...
switch particle_type
  case 'sphere'
    ...
    % Describe the shape
    shape = ott.shapes.Shape.simple('sphere', width/3);
  ...
  case 'cylinder'
    ...
    % Describe the shape
    shape = ott.shapes.Shape.simple('cylinder', [0.1*width, 0.5*width]);
  ...
  case 'cube'
    ...
    % Describe the shape
    shape = ott.shapes.Shape.simple('cube', 0.4*width);
...
for ii=1:numt
  ...
  grid on
  axis equal
  axis([-1,1,-1,1,-1,1]*(width/2))
  title('Particle trajectory and orientation with time.')
  view(3)
  movieframe(ii)=getframe(1);

  % To see the trajectory & orientation change in a 2-D Plot
%   figure(3)
% 
%   hold on
% 
